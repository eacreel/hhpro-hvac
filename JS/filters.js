...
const Filters = (function () {
    ...
    function populateMpsDropdowns() {
        ...
        // Electric Heat kW: add "None" option at the top, then numeric values
    ...
    function updateMpsAvailableOptions() {
        ...
        // For each filter, find valid values given all other active filters
        var fields = [
        ...
        for (var f = 0; f < fields.length; f++) {
            ...
            if (field.key === "electricHeatKw") {
                // Special handling: add "None" option, and handle NONE current value
            ...
            // Re-read state after each repopulate
            state = getStateMps();
        }
    }

    /**
     * Get valid values for an MPS filter by applying all OTHER active filters.
     */
    function getMpsValidValues(allSystems, state, excludeKey, extractor) {
        ...
        var values = Object.keys(valueSet).map(function (v) {
            ...
            // (prevents "208/1" from becoming 208)
            if (/^-?\d+(\.\d+)?$/.test(v)) {
                return parseFloat(v);
            }
            return v;
    ...
    /**
     * Repopulate the Electric Heat kW dropdown with a "None" option at the top.
     */
    function repopulateSelectWithNone(selectEl, values, formatLabel, currentValue, allSystems, state) {
        ...
        // Check if any systems with null electricHeatKw exist given other filters
        var modifiedForNone = {
            size:              state.size,
            systemType:        state.systemType,
            nominalEfficiency: state.nominalEfficiency,
    ...
    /**
     * Populate a select with optional special first value (like "None").
     */
